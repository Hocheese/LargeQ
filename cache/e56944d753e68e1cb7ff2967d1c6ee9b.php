...
<script>
    ajax("/Article/weather",function(data){
        ...
        for(i in dl){
            if(typeof dl[i].getAttribute=="function"){
    ...
    ajax("/Article/friends",function(data){
        var d=JSON.parse(data);
        for(i in d){
            if(d[i]!=null){
                document.getElementById("friends").innerHTML+='<p><a href="http://'+ d[i].url+'">'+ d[i].name+'</a></p>';
            }
        }
    });
</script>
